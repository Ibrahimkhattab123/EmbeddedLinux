...
void separateEvenOdd(const std::array<T, N> &arr, std::array<T, N> &even,
                     std::array<T, N> &odd, size_t &evenSize, size_t &oddSize) {
  ...
  // n A pair designating the ends of the resulting sequences. Copies each
  // element in the range @p [__first,__last) for which
  auto partitionPoint = std::partition_copy(arr.begin(), arr.end(),
                                            even.begin(), odd.begin(), isEven);

  // To calculte the size of the new arrays, which are sent back using the ref
  // param
...
int main() {
  ...
  // normal for loops, and not ranged, becuase ranged will print out 5 elements
  // and not the calculated elements
